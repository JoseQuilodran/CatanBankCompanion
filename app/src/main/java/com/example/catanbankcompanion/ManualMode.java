...
public class ManualMode extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lots= createLots();
        spaces=createGrid(lots);
        sp=new SplittableRandom();
        randomButton=(ImageButton) findViewById(R.id.resultbutton);
        randomButton.setOnTouchListener((v, event) -> {
            if(event.getAction()==event.ACTION_DOWN) {
                a=sp.nextInt(1,7);
                b=sp.nextInt(1,7);
                Intent t= new Intent(ManualMode.this,DialogActivity.class);
    ...
    private class manualTable{
        int actualType=0;
        int actualNumber=0;
        boolean putNumber=false;//true si se esta colocando un numero
        ImageButton iv =null;
        String[] tipos=new String[]{"ovejas","roca","brick","wheat","wood","desierto"};
        int[] cantidades = new int[]{4,3,3,4,4,1};
        int[] numeros= new int[]{2,3,3,4,4,5,5,6,6,8,8,9,9,10,10,11,11,12};
        @SuppressLint("ClickableViewAccessibility")
        manualTable(HashMap<Integer,Space> spaces){
            iv = (ImageButton)findViewById(R.id.actualtile);
            iv.setOnTouchListener((v, event) -> {
                if(event.getAction()==event.ACTION_DOWN) {
                    if(cantidades[actualType]==0&&actualNumber==0){//ultimo space de cierto tipo puesto.
                        if(actualType==5){//caso final,pasar a number
                            actualType=0;
                            actualNumber=1;}
                        else{actualType++;}//caso comun para tipo
                    }
                    else if(actualNumber!=0&&putNumber){
                        if(actualNumber>17){finalizeMM();}
                        else {actualNumber++;}
                        putNumber=false;
                    }
                    updateView();
            ...
            for (Integer key: lots.keySet()) {
                lots.get(key).imageButton.setVisibility(View.INVISIBLE);
            }
            ...
            changeView(actualType+1,0);
            text.setText("Quedan "+cantidades[actualType]+" "+tipos[actualType]+" por colocar");
        }
        void callMM(Space space){
            if(actualNumber==0){
                if(space.type==0&&space.number==0&&cantidades[actualType]>0){//space no tiene nada aun
                   space.setType(actualType+1);
                   space.setImage(space.type);
                   cantidades[actualType]--;
                   if(space.type==6){thiefIsHere(space);} //colocar ladron automaticamente
                }
                else if(actualType+1==space.type){//space tenia algo,volver atras
                    space.setType(0);
                    space.setImage(space.type);
                    cantidades[actualType]++;}
            }
            else{
                if(space.number==0&&!space.hasThief&&!putNumber){//space no tiene numero asignado aun
                    space.number=numeros[actualNumber-1];
                    space.setNumber(numeros[actualNumber-1]);
                    putNumber=true;
                }
                else if(space.number!=0&&!space.hasThief&&putNumber){//space tenia un numero valido
                    space.number=0;
                    space.setNumber(0);
                    putNumber=false;
                }
            ...
            updateView();
            if((cantidades[actualType]==0&&actualNumber==0)||putNumber){readyView();}
        }
        void updateView(){
            if(actualNumber==0){
                text.setText("Quedan "+cantidades[actualType]+" "+tipos[actualType]+" por poner");
                changeView(actualType+1,0);}
            else if(actualNumber!=0){text.setText("Tocar el espacio donde desea colocar "+numeros[actualNumber-1]);
                changeView(0,numeros[actualNumber-1]);}
            iv.setImageResource(0);
        }
        void readyView(){
            text.setText("Pulse en el tick para continuar, o toque un espacio no confirmado para eliminarlo");
            iv.setImageResource(R.drawable.cckeck);
        }
        void finalizeMM(){
            for (Integer key: lots.keySet()){
                lots.get(key).imageButton.setVisibility(View.VISIBLE);
            }
        ...
        void changeView(int type,int number){
            if(number!=0){
                switch(number){
                    case 2:{iv.setBackgroundResource(R.drawable.c2);break;}
                    case 3:{iv.setBackgroundResource(R.drawable.c3);break;}
                    case 4:{iv.setBackgroundResource(R.drawable.c4);break;}
                    case 5:{iv.setBackgroundResource(R.drawable.c5);break;}
                    case 6:{iv.setBackgroundResource(R.drawable.c6);break;}
                    case 7:{iv.setBackgroundResource(R.drawable.c7);break;}
                    case 8:{iv.setBackgroundResource(R.drawable.c8);break;}
                    case 9:{iv.setBackgroundResource(R.drawable.c9);break;}
                    case 10:{iv.setBackgroundResource(R.drawable.c10);break;}
                    case 11:{iv.setBackgroundResource(R.drawable.c11);break;}
                    case 12:{iv.setBackgroundResource(R.drawable.c12);break;}
                }
            }
            else if(number==0){
                switch(type){
                    case 1:{iv.setBackgroundResource(R.drawable.csheep);break;}
                    case 2:{iv.setBackgroundResource(R.drawable.crock);break;}
                    case 3:{iv.setBackgroundResource(R.drawable.cbrick);break;}
                    case 4:{iv.setBackgroundResource(R.drawable.cwheat);break;}
                    case 5:{iv.setBackgroundResource(R.drawable.cwood);break;}
                    case 6:{iv.setBackgroundResource(R.drawable.cdesert);break;}
                }
            }
            else iv.setBackgroundResource(R.drawable.cnull);
        }
    }
    private class Space{
            ...
            imageButton.setOnTouchListener((v, event) -> {
                ...
                if(event.getAction()== MotionEvent.ACTION_DOWN) {
                    if (actualMode==0) {
                        ...
                        if (Color.alpha(pixel) > 0.5) {
                            mT.callMM(this);
                        }
                    }
                    else{
                        int pixel = bitmap.getPixel(x,y);
                        if(Color.alpha(pixel)>0.5&&actualMode==2){
                            thiefIsHere(this);
                            actualMode=1;
                            thiefButton.setBackgroundColor(Color.WHITE);
                        }
                    }
                }
                return false;
